...
(function(){
    ...
    /*if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
        jQuery('.mobileScreen-5544345676867').show();
        jQuery('.screenWeb-342342425').hide();
    } else{
        jQuery('.screenWeb-342342425').show();
        jQuery('.mobileScreen-5544345676867').hide();
    }*/
    jQuery('.screenWeb-342342425').show();
    jQuery('.mobileScreen-5544345676867').hide();
    responsiveOrder();
...
function miniModal(){
    jQuery('.td-contact-area-button-54564316 .td-button-4548264').on('click',function(e){
        e.preventDefault();
        jQuery('.contID-126726137213').fadeIn('slow');
    });
    jQuery('.contID-126726137213 .close-this').on('click',function(){
        jQuery('.contID-126726137213').fadeOut('slow');
    });
}
function stickImage(){
    if (jQuery(window).width()<600) {
    var imgElem = jQuery('.td-additional-fields-responsive-684654 > .wpb_column > .wpb_wrapper > div:nth-of-type(11) .td_module_4 .td-module-thumb img'),
        imgElem2 = jQuery('.td-additional-fields-responsive-684654 > .wpb_column > .wpb_wrapper > div:nth-of-type(11) .td_module_6 .td-module-thumb img'),
        imgSrc = imgElem.attr('src'),
        imgSrc = imgSrc.replace(/-324x235/,'');

    imgElem.parent().addClass('bgimage').css({'background-image':'url('+imgSrc+')'});
    
    imgElem2.each(function(){
        var imgSrce = jQuery(this).attr('src'),
        imgSrce = imgSrce.replace(/-100x70/,'');
        jQuery(this).parent().addClass('bgimage').css({'background-image':'url('+imgSrce+')'});
    });
    }
}
function moveImage(){
    if (jQuery(window).width()<600) {
        jQuery('.td_mycBlock_3432434539 .td_block_wrap .td-big-grid-wrapper .td-big-grid-scroll .td-big-grid-post-3 .td-meta-info-container').before(jQuery('.td_mycBlock_3432434539 .td_block_wrap .td-big-grid-wrapper .td-big-grid-scroll .td-big-grid-post-3 .td-module-thumb'));
    } else {
        jQuery('.td_mycBlock_3432434539 .td_block_wrap .td-big-grid-wrapper .td-big-grid-scroll .td-big-grid-post-3 .td-module-thumb').before(jQuery('.td_mycBlock_3432434539 .td_block_wrap .td-big-grid-wrapper .td-big-grid-scroll .td-big-grid-post-3 .td-meta-info-container'));
    }
}