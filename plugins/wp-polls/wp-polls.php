...
function display_pollvote($poll_id, $display_loading = true) {
    ...
    if(empty($poll_expiry)) {
        ...
        $poll_end_date  = mysql2date(sprintf(__('%s', 'wp-polls'), 'd/m/Y'), gmdate('Y/m/d', $poll_expiry));
        $poll_end_date = 'Votación válida hasta '.$poll_end_date;
    }
...
function display_pollresult($poll_id, $user_voted = '', $display_loading = true, $class = '') {
    ...
    if($poll_question && $poll_answers) {
        ...
        $temp_pollresult .= "<div id=\"polls-$poll_question_id\" class=\"wp-polls $class\">\n";
...
function poll_shortcode( $atts ) {
    ...
    if( ! is_feed() ) {
        ...
        if( $attributes['type'] === 'vote' ) {
            ...
            return display_pollresult( $id, '',true, 'results-type');
        }
    } else {
