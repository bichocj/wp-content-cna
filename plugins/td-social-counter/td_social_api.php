    ...
    public function get_social_counter($service_id, $user_id, $disable_cache = false, $access_token = '') {
        ...
        if ($this->in_cache($service_id, $user_id) === false or $disable_cache === true) { //disable cache here
            ...
            try {
                switch ($service_id) {
                    case 'facebook':
                        $data = @$this->get_json("https://graph.facebook.com/v2.8/$user_id?access_token=$access_token&fields=fan_count");
                        if (array_key_exists('fan_count', $data)) {
                            $buffy_array = (int) $data['fan_count'];
                        }
